...
authRouter.put("/add-new-friend/:id/:newFriendId", (req, res) => {
  let requesterId = req.params.id,
    reciverId = req.params.newFriendId;

  if (requesterId === reciverId)
    res.json("you can't add your self as a frind of you ^_^");

  if (accounts[reciverId - 1] === undefined)
    //note that no need to handle wrong requester id
  ...
  if (isAFrind(requesterId, reciverId)) {
    ...
    if (!isHaveAFrind(requesterId)) accounts[requesterId - 1].friends.shift();

    accounts[requesterId - 1].friends.push(reciverId);

    if (!isHaveAFrind(reciverId)) accounts[reciverId - 1].friends.shift();
...
const isAFrind = (id1, id2) => {
  ...
  if (isHaveAFrind(id1)) {
    accounts[id1 - 1].friends.forEach((friend) => {
      if (friend === id2) return true;
    });
  }
  return false;
};
...
module.exports = { authRouter, isAFrind };
